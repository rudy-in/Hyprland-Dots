...
- Falls du dich wunderst, hier ist eine kurze Demo der AGS-Übersicht [Youtube LINK](https://youtu.be/zY5SLNPBJTs)
...
### 🚩 🏁  Automatische Distro-Hyprland-Installationsskripte klonen und starten
- HINWEIS: Du benötigst das Paket `curl`, damit dies funktioniert

```bash
sh <(curl -L https://raw.githubusercontent.com/JaKooLit/Hyprland-Dots/main/Distro-Hyprland.sh)
```
- Falls du z. B. `fish` oder eine nicht POSIX-kompatible Shell benutzt
```bash
curl -sL https://raw.githubusercontent.com/JaKooLit/Hyprland-Dots/main/Distro-Hyprland.sh | bash
```

- Mit dem obigen Befehl kannst du jetzt automatisch die Distro-Hyprland-Installationsskripte klonen und starten.
- Das Skript wird die Installationsskripte klonen und `install.sh` starten. 😎


### 👁️‍🗨️ Meine Hyprland-Installationsskripte 👁️‍🗨️
- Automatisierte Hyprland-Skripte für eine Distro deiner Wahl. Diese Skripte ziehen bei Bedarf meine vorkonfigurierten Dotfiles
...
- Schau in die Installationsskripte, um zu sehen, welche Pakete benötigt werden... aber zumindest müssen die Hyprland-Pakete installiert sein 😏😏😏 logisch!!
- Dieses Repository wird von den oben genannten Distro-Hyprland-Installationsskripten heruntergeladen, falls du dich entscheidest, vorkonfigurierte Dotfiles zu verwenden.

### 👀 Screenshots 👀
- Alle Screenshots findest du hier [Screenshots](https://github.com/JaKooLit/screenshots/tree/main/Hyprland-ScreenShots)

### 📦 Whats new?
- Um Änderungen leicht nachzuverfolgen, werde ich die [CHANGELOGS](https://github.com/JaKooLit/Hyprland-Dots/wiki/Changelogs) regelmäßig aktualisieren. Screenshots werden hinzugefügt, falls die Änderungen erwähnenswert sind!

> [!NOTE]
> Bitte beachten Sie, dass Kools Dots standardmäßig für ein 2K (1440p) Display ohne Skalierung angepasst/konfiguriert sind.

### 💥 Kopieren / Installation / Update-Anleitung 💥
- [`MORE INFO HERE`](https://github.com/JaKooLit/Hyprland-Dots/wiki/Install_&_Update) 
> [!Note] 
> Das automatische Kopierskript "copy.sh" erstellt Backups der zu kopierenden Verzeichnisse. Es ist dennoch eine gute Idee, manuell ein Backup zu erstellen, falls das Skript fehlschlägt!
- Klone dieses Repository mit Git, wechsle das Verzeichnis, mache die Datei ausführbar und führe das Skript aus:
...
- Automatisches Kopieren/Installieren der vorkonfigurierten Dots (empfohlen für Updates):
...
- Kopieren/Installieren aus den Releases (stabil) (beachte, dass dies eine Version älter als "main" ist):
```bash
chmod +x release.sh
./release.sh
```
